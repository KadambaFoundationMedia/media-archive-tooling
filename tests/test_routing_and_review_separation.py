...
def test_incomplete_but_safe_files_continue_without_human_review(parser_and_planner):
    """Verify that files with missing WHERE or WHAT continue safely without human review (R-022)."""
    parser, planner = parser_and_planner

    # 1. Missing WHERE: continues safely to downstream enrichment
    ...
    # 2. Missing WHAT: retains useful wording + tracking ID and routes to Tool 7
    ...
    assert "tool_7_class_classification" in res_no_what.downstream_routing
