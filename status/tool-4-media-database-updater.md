...
The branch is not yet safe to accept or merge because a caller-controlled stage label is still treated as proof of completed collaboration, and a returned Tool 3 unavailable state is not recognized by the finalization gate.
...
### R-039 — `proposal_mode=finalize` is still accepted as proof of Tool 2/Tool 3 collaboration

`RenameCommitService.commit_file()` derives the stage solely from `files.proposal_mode`, and `_trigger_media_db_sync()` enqueues Tool 4 whenever that stored value equals `finalize`. `proposal_mode` is copied directly from a `RenameProposal`; it is not a completion receipt for the required Tool 2 and Tool 3 stages.

The new portal regression demonstrates the bypass rather than closing it: `test_portal_commit_finalize_proposal_calls_tool_4` manually constructs and saves a `RenameMode.FINALIZE` proposal, creates no Media review or travel review for that tracking ID, and still expects Tool 4 to be called. Therefore an initial/intermediate proposal—or any proposal from another caller—can be labelled `finalize`, approved in the portal, and synchronized without the required collaboration.

Required correction:

- represent successful finalization with durable provenance generated by the actual orchestration path, not only a caller-supplied rename mode;
- bind that provenance to the tracking ID and exact final proposal/current parser state so stale Tool 2/Tool 3 records cannot authorize a later unrelated proposal;
- before automatic Tool 4 enqueue, require proof that the applicable Tool 2 review completed with a usable live result and Tool 3 completed against a verified reference for that finalization attempt;
- preserve the existing rule that human approval may resolve proposal review fields but cannot manufacture missing system-stage completion;
- replace the portal finalize regression with two cases: a merely labelled finalize proposal without collaboration must not call/enqueue Tool 4, while a proposal produced by the genuine completed Tool 1 → Tool 2 → Tool 3 → Tool 1 finalization path must call it exactly once after commit.
...
- add regressions for an existing corrupt/integrity-failing reference, explicit `REFERENCE_UNAVAILABLE`, explicit `PROCESSING_ERROR`, an invalid result, and a valid non-support result that is allowed to continue.
